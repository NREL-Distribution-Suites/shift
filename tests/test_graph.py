...
def distribution_graph():
    ...
    node_1 = NodeModel(name="node_1", location=GeoLocation(-93.33, 45.56))
    node_2 = NodeModel(
        name="node_2",
        location=GeoLocation(-93.33, 45.56),
        assets={DistributionVoltageSource},
    )
    node_3 = NodeModel(
        name="node_3",
        location=GeoLocation(-93.35, 45.58),
        assets={DistributionLoad},
    )
    graph.add_edge(
        node_1,
        node_2,
        edge_data=EdgeModel(name="line-1", edge_type=DistributionTransformer),
    )
    graph.add_edge(
        node_1,
        node_3,
        edge_data=EdgeModel(name="line-2", edge_type=DistributionBranch),
    )
...
def test_node_addition(distribution_graph):
    node = NodeModel(name="node_5", location=GeoLocation(-93.33, 45.56), assets={DistributionLoad})
...
def test_nodes_addition(distribution_graph):
    distribution_graph.add_nodes(
        [
            NodeModel(name="node_4", location=GeoLocation(-93.33, 45.56)),
            NodeModel(name="node_5", location=GeoLocation(-93.33, 45.56)),
        ]
    )
...
def test_edge_addition():
    graph = DistributionGraph()
    node_1 = NodeModel(name="node_1", location=GeoLocation(1, 1))
    node_2 = NodeModel(name="node_2", location=GeoLocation(1, 2))
    graph.add_nodes([node_1, node_2])
    edge_data = EdgeModel(name="line-1", edge_type=DistributionBranch)
    graph.add_edge(
        node_1.name,
        node_2.name,
        edge_data=edge_data,
