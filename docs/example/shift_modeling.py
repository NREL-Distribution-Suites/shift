...
class CustomLoadMapper(EdgeEquipmentMapper):
    ...
    def node_asset_equipment_mapping(self) -> dict[str, object]:
        ...
        for node in self.graph.get_nodes():
            ...
            if DistributionVoltageSource in node.assets:
                ...
                node_equipment_dict[node.name][DistributionVoltageSource] = source.equipment

            if DistributionLoad in node.assets:
                load, active_power, profile = self._build_load(node)
                distribution_systems.add_component(load)
                node_equipment_dict[node.name][DistributionLoad] = load.equipment
        ...
        # print(gdm_graph)
        distribution_systems.to_json(str(BUILD_DIR / "model.json"), overwrite=True)
    ...
    def _build_branches(self, max_load, graph):
        ...
        for edge in graph.edges():
            ...
            voltages = [bus.rated_voltage for bus in buses]
            kva = max_load / 0.9
    ...
    def _build_load(self, node):
        ...
        normalized_type = (parcel.building_type or "").strip().lower()
        supported_building_type = mapped_building_type.get(
            normalized_type,
            ComstockBuildingTypes.WAREHOUSE,
        )
